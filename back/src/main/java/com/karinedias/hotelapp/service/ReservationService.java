...
public class ReservationService {
    ...
    private boolean isReservationUnavailable(Reservation wantedReservation) {
        return StreamSupport.stream(reservationRepo.findAll().spliterator(), false)
                .filter(reservation -> reservation.getHotel().getId() == wantedReservation.getHotel().getId())
                .filter(reservation -> reservation.getNumChambre() == wantedReservation.getNumChambre())
